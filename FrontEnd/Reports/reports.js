...
async function showDailyReport() {
    ...
    const { data } = await axios.post("http://localhost:3500/report/dailyreport", date, { headers: {"Authorization" : token} });
...
async function showMonthlyReport() {
    ...
    const token  = localStorage.getItem('token');
    const { data } = await axios.post("http://localhost:3500/report/monthreport", monthly, { headers: {"Authorization" : token} });
...
async function showPrevReport(){
    const token = localStorage.getItem('token');
    try {
     const {data} = await axios.get('http://localhost:3500/report/prevReport',{
        headers: { "Authorization": token }
     })
