...
function showLeaderboard() {
    ...
    showLeaderboardBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await axios.get('http://localhost:3500/premium/showLeaderBoard', { headers: { "Authorization": token } });
...
async function create() {
    ...
    let {data}= await axios.get(`http://localhost:3500/expense/getexpense?page=${page}&limit=${localStorage.getItem(
        "limit"
...
async function onSubmit(e) {
    e.preventDefault();
        try {
            ...
            const token  = localStorage.getItem('token');
            await axios.post("http://localhost:3500/expense/addexpense", details, { headers: {"Authorization" : token} });
            create();
        }
        catch (err) {
...
async function onDelete(e,id) {
    ...
    try {
        ...
        const response = await axios.delete(`http://localhost:3500/expense/deleteexpense/${id.toString()}`, {
            headers: { "Authorization": token }
        });
...
document.getElementById('rzp-button1').onclick = async function (e) {
    const token = localStorage.getItem('token')
    const response  = await axios.get('http://localhost:3500/purchase/premiummembership', { headers: {"Authorization" : token} });
    ...
    var options =
    {
     ...
     "handler": async function (response) {
        const res = await axios.post('http://localhost:3500/purchase/updatetransactionstatus',{
